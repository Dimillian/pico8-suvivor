...
player = {
	...
	draw_ui=function(self)
		...
		print("🐱 " .. self.kills ..  " ⧗ " .. flr(self.time),  x + 4 ,y + 120, 7)
	end,
	...
	update_level=function(self)
		if self.xp >= self.next_xp then
			...
			self.xp = 0
			self.level += 1
			self.next_xp += (self.level < 20) and 5 or 10
			state = state_lvl_up
		end
	end,
	...
	update_health=function(self)
		if self.health <= 0 then
			local death_count = dget(d_death_count)
			death_count += 1
			dset(d_death_count, death_count)
			if self.kills > dget(d_best_scrore) then
				dset(d_best_scrore, self.kills)
			end
...
world = {
	init=function(self)
		...
		self.chests={}
		self.last_boss = 0
		self.end_boss_time = 600
		self.end_boss_spawned = false
		self.foe_x = 20
	...
	update=function(self, player)
		...
		if self.end_boss_spawned then
			return
		end
		local boss_trigger = 30
		local spawn_max = 90
		local spawn = self.foe_x + player.level
		...
		if #self.foes <= spawn then
			if player.kills > 0 and player.kills > self.last_boss and player.kills % boss_trigger == 0 then
				...
				self:add_foe(player, true, false)
			else
				self:add_foe(player, false, false)
			end
		end
		if flr(player.time) >= self.end_boss_time then
			self.end_boss_spawned = true
			self:add_foe(player, false, true)
		end
	end,

	add_foe=function(self, player, is_boss, is_last_boss)
		local base_speed = 0.15
		local speed = base_speed + (0.0010 * player.level)
		local xp = rnd(1 * ceil(player.level / 5))
			...
			check_death=function(self)
				if self.health <= 0 then
					if not self.is_dead then
						self.is_dead=true
						if is_last_boss then
							local score = dget(d_last_boss_kill)
							score += 1
							dset(d_last_boss_kill, score)
							state = state_win
						end
						sfx(sfx_foe_death)
						player.kills += 1
...
menu_lvl_up = {
	...
	init=function(self)
		self.choices = {
			{target="player", name="speed", subtitle="+15% move speed", sprite=23},
			{target="player", name="health", subtitle="+5 hp", sprite=22},
		...
		self.sword_options_added = false
		self.storm_options_added = false
		self.whip_options_added = false
	end,

	draw=function(self)
		local menu_width=128
		local menu_height=self.height_current
	...
	is_expanded=function(self)
	 return	self.height_current == self.height_max
	end,
	...
	process_lvl_up=function(self, option, player)
		...
		elseif option.target == "whip" then
			if option.name == "whip" then
				player.whip_on = true
			elseif option.name == "delay" and player.whip_delay_x < 80 then
...
function draw_stats()
	local s = "score:" .. player.kills
	print(s, 64-#s*2, 70, 7)

	s = "best:" .. dget(d_best_scrore)
	print(s, 64-#s*2, 80, 7)
		
	s = "death:" .. dget(d_death_count)
	print(s, 64-#s*2, 90, 7)
		
	s = "win:" .. dget(d_last_boss_kill)	
...

...

__label__
